...
export function DashboardPage() {
  ...
  // Listen for data updates from upload page
  useEffect(() => {
    const handleUpdate = () => fetchDashboardData();
    window.addEventListener('kread:data-updated', handleUpdate);
    return () => window.removeEventListener('kread:data-updated', handleUpdate);
  }, [periodType]);
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const computeChange = (current: number, previous: number) => {
        if (previous === 0) return { absolute: current, percentage: 0, trend: 'neutral' as const };
      ...
      setKpis({
        total_restaurants: { current: totalRestaurants || 0 },
        total_sales: {
          current: currentTotals['sales'] || 0,
          previous: previousTotals['sales'] || 0,
          change: computeChange(currentTotals['sales'] || 0, previousTotals['sales'] || 0),
        },
        total_orders: {
          current: currentTotals['delivered_orders'] || 0,
          previous: previousTotals['delivered_orders'] || 0,
          change: computeChange(currentTotals['delivered_orders'] || 0, previousTotals['delivered_orders'] || 0),
        },
        average_aov: {
          current: currentTotals['average_order_value'] || 0,
          previous: previousTotals['average_order_value'] || 0,
          change: computeChange(currentTotals['average_order_value'] || 0, previousTotals['average_order_value'] || 0),
        },
        average_roi: {
          current: currentTotals['ads_roi'] || 0,
          previous: previousTotals['ads_roi'] || 0,
          change: computeChange(currentTotals['ads_roi'] || 0, previousTotals['ads_roi'] || 0),
        },
        total_ad_spend: {
          current: currentTotals['ads_spend'] || 0,
          previous: previousTotals['ads_spend'] || 0,
          change: computeChange(currentTotals['ads_spend'] || 0, previousTotals['ads_spend'] || 0),
        },
        total_offer_sales: {
          current: currentTotals['gross_sales_from_offers'] || 0,
          previous: previousTotals['gross_sales_from_offers'] || 0,
          change: computeChange(currentTotals['gross_sales_from_offers'] || 0, previousTotals['gross_sales_from_offers'] || 0),
        },
      });
      ...
      setAlerts((alertsData || []).map(a => ({
        ...
        restaurantName: a.restaurants?.name,
        metricName: a.metric_name,
        severity: a.severity,
        currentValue: a.current_value,
        previousValue: a.previous_value,
        percentageDrop: a.percentage_drop,
      ...
      const restaurantSales = new Map<string, { name: string; sales: number }>();
      for (const m of currentMetrics || []) {
        const existing = restaurantSales.get(m.restaurant_id) || { name: '', sales: 0 };
        restaurantSales.set(m.restaurant_id, {
          name: existing.name,
          sales: existing.sales + (Number(m.sales) || 0),
        });
      }

      // Fetch restaurant names
      const restaurantIds = Array.from(restaurantSales.keys());
      if (restaurantIds.length > 0) {
        ...
        for (const r of restaurants || []) {
          const existing = restaurantSales.get(r.id);
          if (existing) existing.name = r.name;
        }
      }

      const sortedRestaurants = Array.from(restaurantSales.entries())
        .sort((a, b) => b[1].sales - a[1].sales);

      setTopPerformers(sortedRestaurants.slice(0, 5).map(([id, data], i) => ({
        rank: i + 1,
        restaurant_id: id,
        restaurant_name: data.name,
        metric_value: data.sales,
      })));

      setBottomPerformers(sortedRestaurants.slice(-5).reverse().map(([id, data], i) => ({
        rank: sortedRestaurants.length - 4 + i,
        restaurant_id: id,
        restaurant_name: data.name,
        metric_value: data.sales,
      })));

      // Update KPIs with active alerts
      ...
      setKpis(prev => ({
        ...prev,
        active_alerts: { current: activeAlertsCount || 0 },
      }));
